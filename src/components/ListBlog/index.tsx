  ...
  return(
    <div className={styles.card}>
      ...
      <div className={styles.content}>
        ...
        <Link href={data.link} ><h3 className={styles.title}>{data.title}</h3></Link>
...
export const ListBlog = ({data} : {data: BlogProps[]}) => {
  return (
    <div className={styles.listBlog}>
      {
        data.map((item, index) => <div className={styles.blog} key={index.toString()} ><CardBlog data={item} /></div>)
      }
    </div>
  )
}
